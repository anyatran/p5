...
// Penner's Easing Functions:
  ...
  // quarter-cycle sine
  p5.Ease.prototype.sineIn = function(_x, p) {
  	return(p.sin((_x - 1) * p.HALF_PI) + 1);
  }

  // quarter-cycle cosine
  p5.Ease.prototype.sineOut = function(_x, p) {
  	return(p.sin(_x * HALF_PI));
  }

  // half sine
  p5.Ease.prototype.sineInOut = function(_x, p) {
  	return(0.5 * (1 - p.cos(_x * p.PI)));
  }

  // shifted quadrant IV of unit circle
  p5.Ease.prototype.circularIn = function(_x, p) {
  	return(1 - p.sqrt(1 - (_x * _x)));
  }

  // shifted quadrant II of unit circle
  p5.Ease.prototype.circularOut = function(_x, p) {
  	return(p.sqrt((2 - _x) * _x));
  }

  // piecewise circular function
  // y = (1/2)(1 - sqrt(1 - 4x^2))           ; [0, 0.5)
  // y = (1/2)(sqrt(-(2x - 3)*(2x - 1)) + 1) ; [0.5, 1]
  p5.Ease.prototype.circularInOut = function(_x, p) {
  	if(_x < 0.5)
  	{
  		return(0.5 * (1 - p.sqrt(1 - 4 * (_x * _x))));
  	}
  	else
  	{
  		return(0.5 * (p.sqrt(-((2 * _x) - 3) * ((2 * _x) - 1)) + 1));
  	}
  }

  // exponential function y = 2^(10(x - 1))
  p5.Ease.prototype.exponentialIn = function(_x, p) {
  	return((_x == 0.0) ? _x : p.pow(2, 10 * (_x - 1)));
  }

  // exponential function y = -2^(-10x) + 1
  p5.Ease.prototype.exponentialOut = function(_x, p) {
  	return((_x == 1.0) ? _x : 1 - p.pow(2, -10 * _x));
  }

  // piecewise exponential
  // y = (1/2)2^(10(2x - 1))         ; [0,0.5)
  // y = -(1/2)*2^(-10(2x - 1))) + 1 ; [0.5,1]
  p5.Ease.prototype.exponentialInOut = function(_x, p) {
  	if(_x == 0.0 || _x == 1.0) return _x;

  	if(_x < 0.5)
  	{
  		return(0.5 * p.pow(2, (20 * _x) - 10));
  	}
  	else
  	{
  		return(-0.5 * p.pow(2, (-20 * _x) + 10) + 1);
  	}
  }

  // damped sine wave y = sin(13pi/2*x)*p.pow(2, 10 * (x - 1))
  p5.Ease.prototype.elasticIn = function(_x, p) {
  	return(p.sin(13 * p.HALF_PI * _x) * p.pow(2, 10 * (_x - 1)));
  }

  // damped sine wave y = sin(-13pi/2*(x + 1))*pow(2, -10x) + 1
  p5.Ease.prototype.elasticOut = function(_x, p) {
  	return(p.sin(-13 * p.HALF_PI * (_x + 1)) * p.pow(2, -10 * _x) + 1);
  }

  // piecewise damped sine wave:
  // y = (1/2)*sin(13pi/2*(2*x))*pow(2, 10 * ((2*x) - 1))      ; [0,0.5)
  // y = (1/2)*(sin(-13pi/2*((2x-1)+1))*pow(2,-10(2*x-1)) + 2) ; [0.5, 1]
  p5.Ease.prototype.elasticInOut = function(_x, p) {
  	if(_x < 0.5)
  	{
  		return(0.5 * p.sin(13 * p.HALF_PI * (2 * _x)) * p.pow(2, 10 * ((2 * _x) - 1)));
  	}
  	else
  	{
  		return(0.5 * (p.sin(-13 * p.HALF_PI * ((2 * _x - 1) + 1)) * p.pow(2, -10 * (2 * _x - 1)) + 2));
  	}
  }

  // overshooting cubic y = x^3-x*sin(x*pi)
  p5.Ease.prototype.backIn = function(_x, p) {
  	return(_x * _x * _x - _x * p.sin(_x * p.PI));
  }

  // overshooting cubic y = 1-((1-x)^3-(1-x)*p.sin((1-x)*pi))
  p5.Ease.prototype.backOut = function(_x, p) {
  	var f = (1 - _x);
  	return(1 - (f * f * f - f * p.sin(f * p.PI)));
  }

  // piecewise overshooting cubic function:
  // y = (1/2)*((2x)^3-(2x)*sin(2*x*pi))           ; [0, 0.5)
  // y = (1/2)*(1-((1-x)^3-(1-x)*sin((1-x)*pi))+1) ; [0.5, 1]
  p5.Ease.prototype.backInOut = function(_x, p) {
  	if(_x < 0.5)
  	{
  		var f = 2 * _x;
  		return(0.5 * (f * f * f - f * p.sin(f * p.PI)));
  	}
  	else
  	{
  		var f = (1 - (2*_x - 1));
  		return(0.5 * (1 - (f * f * f - f * p.sin(f * p.PI))) + 0.5);
  	}
  }
